...
Prophet.turn = function turn(_this) {
	...
	if(enemies.length>0) {
		...
		for(let i = temp_goals.length - 1; i>=0; i--) {
			...
			if (nav.distSquared(loc, [_this.me.x, _this.me.y])<=64) {
	...
	for(let i = lattice_goals.length - 1; i>=0; i--) {
		if(lattice_goals.length === 0) {
			break;
		}
		for(let j = 0; j<friends.length; j++) {
			let friend = friends[j];
			if(friend.id === _this.me.id) {
				continue;
			}
			//_this.log("lattice goals:  " + lattice_goals);
			//_this.log("i:  "+i);
			if(lattice_goals[i][0] === friend.x && lattice_goals[i][1] === friend.y) {
				lattice_goals.splice(i,1);
				//_this.log(lattice_goals);
				break;
			}
		}
	}
	if(lattice_goals.length === 0 && (_this.me.x + _this.me.y)%2 === 0) {
		lattice_goals.push(nearestLatticeLocation(lattice_locations, [_this.me.x, _this.me.y]));
	}
	...
	if(lattice_goals.length> 0) {
		let bfsMap = nav.breadthFirstSearch(lattice_goals, map_copy, Prophet.moveDirs);
		if(bfsMap[_this.me.y][_this.me.x].length > 0) {
			let newLocation = bfsMap[_this.me.y][_this.me.x][Math.floor(Math.random()*bfsMap[_this.me.y][_this.me.x].length)];
			return _this.move(newLocation[0] - _this.me.x, newLocation[1] - _this.me.y);
		}
	...
	//get rid of code to move towards enemy castles
