...
Castle.turn = function turn(_this){
	...
	if(_this.karbonite>= SPECS.UNITS[bestBuild].CONSTRUCTION_KARBONITE && _this.fuel>= SPECS.UNITS[bestBuild].CONSTRUCTION_FUEL &&( (karbs_built>=max_karbs/2 && fuels_built>=max_fuels/2) || ( (enemies.length>0 || (numCrusaders*20+numProphets*25+numPreachers*30) >=120)))){
	...
	else if (karbs_built<max_karbs/1.5 && _this.karbonite>=60){
		// build a karb miner.
		for(let i = 0; i<Castle.buildDirs.length; i++){
			let x = Castle.buildDirs[i][0]+_this.me.x;
			let y = Castle.buildDirs[i][1]+_this.me.y;
